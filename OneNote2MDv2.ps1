...
# Output path that will contain your resulting Notes structure. ex. 'c:\temp\notes'
$notesdestpath = "C:\temp\output"

## OneNote Notebook Name to export
# $notebookName = "test"
$notebookName = "Dynamics ACE Program Guide"

## Notebook Toplevel Group Section to Export
# $notebookGroupSection = "Generic Guide"
$notebookGroupSection = "CE Guide"
...
Function ProcessSections ($group, $FilePath) {
    foreach ($section in $group.Section) {
        ...
        New-Item -Path "$($FilePath)" -Name "$($sectionFileName)" -ItemType "directory" -ErrorAction SilentlyContinue | Out-Null
        ...
        foreach ($page in $section.Page) {
            ...
            try {
                ...
                pandoc.exe -f  docx -t $converter -i $fullexportpath -o "$($fullfilepathwithoutextension).md" --wrap=none --markdown-headings=atx --extract-media="$($mediaPath)"
            }
            catch {
...
if (Test-Path -Path $notesdestpath) {
    ...
    if ($notebook.name -eq $notebookName){
        " "
        "Processing Notebook: " + $notebook.Name
        $notebookFileName = "$($notebook.Name)" | Remove-InvalidFileNameChars
        New-Item -Path "$($notesdestpath)\" -Name "$($notebookFileName)" -ItemType "directory" -ErrorAction SilentlyContinue | Out-Null
        $NotebookFilePath = "$($notesdestpath)\$($notebookFileName)"
        $levelsfromroot = 0
       
        New-Item -Path "$($NotebookFilePath)" -Name "docx" -ItemType "directory" -ErrorAction SilentlyContinue | Out-Null
        ...
        if ($sectiongroup1.Name -eq $notebookGroupSection){
            $levelsfromroot = 1
            if ($sectiongroup1.isRecycleBin -ne 'true') {
                "# " + $sectiongroup1.Name
                $sectiongroupFileName1 = "$($sectiongroup1.Name)" | Remove-InvalidFileNameChars
                New-Item -Path "$($notesdestpath)\$($notebookFileName)" -Name "$($sectiongroupFileName1)" -ItemType "directory" -ErrorAction SilentlyContinue | Out-Null
                ...
                foreach ($sectiongroup2 in $sectiongroup1.SectionGroup) {
                    ...
                    if ($sectiongroup2.isRecycleBin -ne 'true') {
                        ...
                        New-Item -Path "$($notesdestpath)\$($notebookFileName)\$($sectiongroupFileName1)" -Name "$($sectiongroupFileName2)" -ItemType "directory" -ErrorAction SilentlyContinue | Out-Null
                        ...
                        foreach ($sectiongroup3 in $sectiongroup2.SectionGroup) {
                            ...
                            if ($sectiongroup3.isRecycleBin -ne 'true') {
                                ...
                                New-Item -Path "$($notesdestpath)\$($notebookFileName)\$($sectiongroupFileName1)\$($sectiongroupFileName2)" -Name "$($sectiongroupFileName3)" -ItemType "directory" -ErrorAction SilentlyContinue | Out-Null
                                ...
                                foreach ($sectiongroup4 in $sectiongroup3.SectionGroup) {
                                    ...
                                    if ($sectiongroup4.isRecycleBin -ne 'true') {
                                        ...
                                        New-Item -Path "$($notesdestpath)\$($notebookFileName)\$($sectiongroupFileName1)\$($sectiongroupFileName2)\$($sectiongroupFileName3)" -Name "$($sectiongroupFileName4)" -ItemType "directory" -ErrorAction SilentlyContinue | Out-Null
                                        ...
                                        foreach ($sectiongroup5 in $sectiongroup4.SectionGroup) {
                                            ...
                                            if ($sectiongroup5.isRecycleBin -ne 'true') {
                                                ...
                                                New-Item -Path "$($notesdestpath)\$($notebookFileName)\$($sectiongroupFileName1)\$($sectiongroupFileName2)\$($sectiongroupFileName3)\$($sectiongroupFileName4)" -Name "$($sectiongroupFileName5)" -ItemType "directory" -ErrorAction SilentlyContinue | Out-Null
    ...
    # if ($keepdocx -ne 2) remove docx folder
    if ($keepdocx -ne 2) {
        Remove-Item -path "$($NotebookFilePath)/docx" -Force -ErrorAction SilentlyContinue
    }
